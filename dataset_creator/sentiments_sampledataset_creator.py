...
output_folder = "/Users/fidenciofernandez/OneDrive/IMT/M1S2/DeepLearning/Proj/SentimentsSampledDataSet"

# Step 2: Create Output Folder if Not Exists
os.makedirs(output_folder, exist_ok=True)

# Step 3: Extract Audio Segments and Create CSV
def extract_audio_segments(audio_path, sentiment, output_folder, segment_duration=10, num_samples=15, starting_number=1):
...
for sentiment, audio_path in input_audios.items():
    csv_data, overall_segment_count = extract_audio_segments(audio_path, sentiment, output_folder, num_samples=15, starting_number=overall_segment_count)
    all_csv_data.extend(csv_data)

# Shuffle the appearances of each music type in the CSV file
random.shuffle(all_csv_data)
...
print(f"Small dataset with shuffled appearances created successfully at: {csv_file_path}")
